...
/**
 * @brief Given a timeframe, polls for those stock bars at an appropriate interval.
 *
 * @param timeframe
 * @param history
 */
void StockHistory::RetrieveBars(const Timeframe timeframe, std::shared_ptr<History> history) {
  ...
  auto f = [sym, a, h, timeframe]() {
    ...
    std::cout << "Retrieving Stock Bars for " << sym << ", " << t << " timeframe every " << interval / 60 << " minutes"
              << std::endl;
